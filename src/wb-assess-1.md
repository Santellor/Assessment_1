...
```
console.log is used for debugging. It returns a value generated by the code to the local terminal, but otherwise has no affect on the code. 
Return is used within functions. For an argument/input given, there is an output, and that output is designated with return. 
```

**_What's the difference between defining and calling a function?_**

```
Defining a function is the act of creating and modifying a parameter within a function to conform arguments to the desired output or outputs, which are then returned. 
Calling a function activates the previously written code, and often provides an argument to the function, returning an output. 
```
...
```
A block of characters that are `strung ` together between backticks, quotes, or apostrophes
```

**_Number_**

```
A number value that is utilized for mathematics and logic gates. NaN and Infinity are also included here
```

**_Boolean_**

```
A value that is either true, or false. Many values in JavaScript are interpreted as truthy or falsy, and can also be accepted in Boolean comparisons
```

**_Array_**

```
A collection of values that are assigned a location according to an index
```
